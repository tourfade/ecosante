...
public class SurgeriesEditorDialog extends DialogFragment implements View.OnLongClickListener {
    ...
    public SurgeriesEditorDialog(@NonNull ExtraData root, AppCompatCheckBox buttonView, boolean editable) {
        this.rootData = root;
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        alertDialogBuilder.setTitle(getString(R.string.surgeries));
        alertDialogBuilder.setCancelable(true);
        alertDialogBuilder.setView(R.layout.surgeries_dialog);
        alertDialogBuilder.setIcon(R.drawable.surgery);
    ...
    void initVars(AlertDialog d) {
        actOracle = new ActOracleAdapter(getActivity());

        date = d.findViewById(R.id.surgery_date);
        date.setEnabled(editable);
        act = d.findViewById(R.id.act);
        act.setEnabled(editable);
        act.setAdapter(actOracle);
        surgeries = d.findViewById(R.id.surgeries);
    ...
    void initListeners(AlertDialog d) {
        if (editable) {
            ...
            positiveButton.setOnClickListener(v -> {
                if(Utils.isNullOrEmpty(act.getText().toString())) {
                    act.setError("Donner le nom de chirurgie");
                    return;
                }
    ...
    void initValues() {
        ...
        if (rootData.items.size() > 0) {
            ...
            source.setText(rootData.items.size() +" Chirurgies. dernier -> le " + Utils.format(new Date(last.date)));
        } else {
