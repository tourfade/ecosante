...
public class AllergiesEditorDialog extends DialogFragment implements View.OnLongClickListener {
    ...
    private LinearLayout allergies;
    private AllergenOracleAdapter allergenOracle;
    private AutoCompleteTextView allergen;


    public AllergiesEditorDialog(@NonNull ExtraData root, AppCompatCheckBox buttonView, boolean editable) {
        this.rootData = root;
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        alertDialogBuilder.setTitle(getString(R.string.allergies));
        alertDialogBuilder.setCancelable(true);
        alertDialogBuilder.setView(R.layout.allergies_dialog);
        alertDialogBuilder.setIcon(R.drawable.allergy);
    ...
    void initVars(AlertDialog d) {
        allergenOracle = new AllergenOracleAdapter(getActivity());

        date = d.findViewById(R.id.allergy_date);
        date.setEnabled(editable);
        allergen = d.findViewById(R.id.allergens);
        allergen.setEnabled(editable);
        allergen.setAdapter(allergenOracle);
        allergies = d.findViewById(R.id.allergies);
    ...
    void initListeners(AlertDialog d) {
        if (editable) {
            ...
            positiveButton.setOnClickListener(v -> {
                if(Utils.isNullOrEmpty(allergen.getText().toString())) {
                    allergen.setError("Donner le nom de chirurgie");
                    return;
                }
                ...
                ed.name = allergen.getText().toString();
    ...
    void initValues() {
        ...
        if (rootData.items.size() > 0) {
            ...
            source.setText(rootData.items.size() +" Allergies " + last.name);
        } else {
